...
// ─── State initialiser (single source — fixes the ID-mismatch bug) ────────────

const initState = () => {
  try {
    const saved   = localStorage.getItem("mtg-decks");
    const decks   = saved ? JSON.parse(saved) : [newDeck("My First Deck")];
    const savedId = localStorage.getItem("mtg-active");
    const activeId = decks.find(d => d.id === savedId)?.id ?? decks[0]?.id;
    return { decks, activeId };
  } catch {
    const d = newDeck("My First Deck");
...
const Tag = ({ children, color=C.muted }) => (
  <span style={{ fontSize:10, fontWeight:700, letterSpacing:.8, color, background:`${color}22`, padding:"2px 6px", borderRadius:4, textTransform:"uppercase", flexShrink:0 }}>
    {children}
  </span>
);
...
const ColorPip = ({ color }) => {
  ...
  return (
    <span title={m.label} style={{ display:"inline-flex", alignItems:"center", justifyContent:"center", width:18, height:18, borderRadius:"50%", background:m.bg, color:m.fg, fontSize:10, fontWeight:700, border:"1px solid rgba(255,255,255,.15)", flexShrink:0 }}>
      {m.emoji}
    </span>
  );
};
...
const Btn = ({ children, onClick, disabled, style={}, variant="primary" }) => (
  <button onClick={onClick} disabled={disabled} style={{
    ...
    background: variant==="primary" ? "linear-gradient(135deg,#f59e0b,#d97706)" :
                variant==="danger"  ? "linear-gradient(135deg,#ef4444,#b91c1c)" :
                variant==="purple"  ? "linear-gradient(135deg,#a855f7,#7c3aed)" : "transparent",
    color: variant==="primary" ? "#0a0f1e" :
           variant==="danger"  ? "#fff"    :
           variant==="purple"  ? "#fff"    : C.gold,
...
// ─── Deck Selector Bar (shown at top of Scan tab) ─────────────────────────────

const DeckSelectBar = ({ deck, deckCount, onSwitch }) => {
  const total  = deck.cards.reduce((s,d) => s+d.count, 0);
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize:10, fontWeight:700, letterSpacing:1.4, color:C.muted, textTransform:"uppercase", marginBottom:8 }}>
        Adding cards to
      </div>
      <div style={{ display:"flex", alignItems:"center", gap:12 }}>
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ fontSize:18, fontWeight:800, color:C.gold, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
            {deck.name}
          </div>
          <div style={{ fontSize:12, color:C.sub, marginTop:2 }}>
            {total} / {target} cards · {deck.format==="commander" ? "👑 Commander" : "60-Card"}
          </div>
          <div style={{ marginTop:8, height:5, background:C.card2, borderRadius:3, overflow:"hidden" }}>
            <div style={{ height:"100%", width:`${pct}%`, borderRadius:3, transition:"width .4s", background: total>=target ? `linear-gradient(90deg,${C.green},#34d399)` : deck.format==="commander" ? "linear-gradient(90deg,#a855f7,#7c3aed)" : `linear-gradient(90deg,${C.gold},#d97706)` }} />
          </div>
        </div>
        <button onClick={onSwitch} style={{
          ...
          textAlign:"center", lineHeight:1.4, minHeight:54, minWidth:70,
        }}>
          🃏<br/>Switch<br/>Deck
        </button>
      </div>
...
// ─── Decks Tab (full deck management) ────────────────────────────────────────

const DecksTab = ({ decks, activeId, onSwitch, onCreate, onUpdate, onDelete }) => {
  ...
  const saveEdit = () => {
    if (editVal.trim()) onUpdate(editId, { name: editVal.trim() });
    setEditId(null);
  };
  const saveNotes = () => {
    onUpdate(notesId, { notes: notesVal });
    setNotesId(null);
  };
  ...
  return (
    <div style={{ padding:"16px 16px 0" }}>
      ...
      <button onClick={onCreate} style={{
        ...touch, width:"100%", marginBottom:14,
        background:"linear-gradient(135deg,#f59e0b,#d97706)",
        border:"none", borderRadius:14, padding:"16px",
        display:"flex", alignItems:"center", justifyContent:"center", gap:10,
        fontSize:16, fontWeight:800, color:"#0a0f1e", minHeight:56,
      }}>
        <span style={{ fontSize:22 }}>+</span> Create New Deck
      </button>

      <div style={{ fontSize:11, fontWeight:700, letterSpacing:1.4, color:C.muted, textTransform:"uppercase", marginBottom:10 }}>
      ...
      {decks.map(deck => {
        ...
        const isNotes   = notesId === deck.id;
        const isDeleting = delId === deck.id;

        return (
          <div key={deck.id} style={{
            background: isActive ? `${C.gold}0f` : C.card,
            border:`2px solid ${isActive ? C.gold+"55" : C.border}`,
            ...
            <div style={{ padding:"14px 16px" }}>
              <div style={{ display:"flex", alignItems:"flex-start", gap:10 }}>
                {/* Active indicator */}
                <div style={{ width:10, height:10, borderRadius:"50%", background: isActive ? C.gold : C.card2, border:`2px solid ${isActive ? C.gold : C.border}`, marginTop:5, flexShrink:0 }} />
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  {isEditing ? (
                    <div style={{ display:"flex", gap:8, marginBottom:8 }}>
                      <input
                        ...
                        autoFocus
                        style={{ flex:1, background:C.card2, border:`1px solid ${C.gold}`, borderRadius:8, color:C.text, padding:"8px 12px", fontSize:15, fontWeight:700, outline:"none", minHeight:40 }}
                      />
                      <Btn onClick={saveEdit} style={{ padding:"8px 14px", fontSize:13 }}>Save</Btn>
                    </div>
                  ) : (
                    <div style={{ fontSize:17, fontWeight:800, color:isActive ? C.gold : C.text, marginBottom:4, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                      {isActive ? "✓ " : ""}{deck.name}
                    </div>
                  )}
                  ...
                  <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:8 }}>
                    <Tag color={deck.format==="commander" ? C.purple : C.gold}>
                      {deck.format==="commander" ? "👑 Commander" : "60-Card"}
                    </Tag>
                    <span style={{ fontSize:11, color:C.sub }}>{total}/{target} cards</span>
                    {deck.commanders.length > 0 && (
                      <span style={{ fontSize:11, color:"#d8b4fe" }}>
                  ...
                  {/* Progress bar */}
                  <div style={{ height:4, background:C.card2, borderRadius:2, overflow:"hidden", marginBottom:10 }}>
                    <div style={{ height:"100%", width:`${Math.min(100,(total/target)*100)}%`, borderRadius:2, background: total>=target ? `linear-gradient(90deg,${C.green},#34d399)` : deck.format==="commander" ? "linear-gradient(90deg,#a855f7,#7c3aed)" : `linear-gradient(90deg,${C.gold},#d97706)` }} />
                  </div>

                  {/* Notes preview */}
                  {deck.notes && !isNotes && (
                    <div style={{ fontSize:12, color:C.muted, lineHeight:1.5, marginBottom:8, fontStyle:"italic" }}>
                      📝 {deck.notes.length > 80 ? deck.notes.slice(0,80)+"…" : deck.notes}
                    </div>
                  )}
                  ...
                  {isNotes && (
                    <div style={{ marginBottom:8 }}>
                      <textarea
                        ...
                        placeholder="Strategy notes, card ideas, goals…"
                        style={{ width:"100%", minHeight:90, background:C.card2, border:`1px solid ${C.gold}`, borderRadius:8, color:C.text, padding:"10px 12px", fontSize:13, outline:"none", resize:"none", boxSizing:"border-box", lineHeight:1.6 }}
                      />
                      <div style={{ display:"flex", gap:8, marginTop:6 }}>
                        <Btn onClick={saveNotes} style={{ fontSize:12, padding:"8px 14px" }}>Save Notes</Btn>
                        <Btn variant="secondary" onClick={() => setNotesId(null)} style={{ fontSize:12, padding:"8px 14px" }}>Cancel</Btn>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginTop:4 }}>
                {!isActive && (
                  <Btn onClick={() => onSwitch(deck.id)} style={{ fontSize:13, padding:"8px 14px", flex:1 }}>
                    Switch to this deck
                  </Btn>
                )}
                <button onClick={() => { setEditId(deck.id); setEditVal(deck.name); }} style={{
                  ...touch, fontSize:12, color:C.sub, background:C.card2, border:`1px solid ${C.border}`, borderRadius:8, padding:"8px 12px", minHeight:38,
                }}>✏️ Rename</button>
                <button onClick={() => { setNotesId(deck.id); setNotesVal(deck.notes||""); }} style={{
                  ...touch, fontSize:12, color:C.sub, background:C.card2, border:`1px solid ${C.border}`, borderRadius:8, padding:"8px 12px", minHeight:38,
                }}>📝 Notes</button>
                {decks.length > 1 && !isDeleting && (
                  <button onClick={() => setDelId(deck.id)} style={{
                    ...touch, fontSize:12, color:C.red, background:`${C.red}15`, border:`1px solid ${C.red}33`, borderRadius:8, padding:"8px 12px", minHeight:38,
                  }}>🗑</button>
                )}
              </div>
              ...
              {/* Delete confirm */}
              {isDeleting && (
                <div style={{ marginTop:10, background:`${C.red}12`, border:`1px solid ${C.red}33`, borderRadius:10, padding:"12px 14px" }}>
                  <div style={{ fontSize:13, color:C.red, fontWeight:600, marginBottom:8 }}>
                    Delete "{deck.name}"? This can't be undone.
                  </div>
                  <div style={{ display:"flex", gap:8 }}>
                    <Btn variant="danger" onClick={() => { onDelete(deck.id); setDelId(null); }} style={{ fontSize:13, padding:"8px 14px" }}>Delete</Btn>
                    <Btn variant="secondary" onClick={() => setDelId(null)} style={{ fontSize:13, padding:"8px 14px" }}>Cancel</Btn>
                  </div>
                </div>
              )}
            </div>
...
const CommanderHeroes = ({ commanders, onRemove, onPreview, canAddSecond }) => {
  ...
  return (
    <div style={{ marginBottom:12 }}>
      {commanders.map((cmdr, idx) => {
        ...
        return (
          <div key={cmdr.id} style={{ background:"linear-gradient(135deg,#1a0a3a,#2d0a5a)", border:"2px solid #a855f744", borderRadius:14, padding:14, marginBottom:8, position:"relative", overflow:"hidden" }}>
              ...
              <div style={{ flex:1 }}>
                ...
                {identity.length > 0 && (
                  <div style={{ display:"flex", gap:4, marginBottom:8, alignItems:"center" }}>
                    <span style={{ fontSize:10, color:C.muted }}>Identity:</span>
      ...
      {canAddSecond && (
        <div style={{ background:"rgba(168,85,247,.08)", border:"1px dashed rgba(168,85,247,.4)", borderRadius:10, padding:"10px 14px", fontSize:12, color:"#a855f7" }}>
          👑 Scan or search a second Legendary Creature and tap "Make Commander" to add a Partner
        </div>
      )}
    </div>
  );
      ...
      const res = await fetch(ANTHROPIC_API, {
        ...
        body:JSON.stringify({
          ...
          messages:[{ role:"user", content:[
            ...
            { type:"text",  text:'MTG card scan. Is a Magic card clearly centred and readable? Reply ONLY JSON: {"name":"exact card name"} or {"name":null}.' }
          ]}]
        })
      });
      if (res.ok) {
    ...
    <div>
      ...
      <div style={{ position:"relative", borderRadius:12, overflow:"hidden", background:"#000", aspectRatio:"4/3" }}>
        ...
        <div style={{ position:"absolute", inset:0, display:"flex", alignItems:"center", justifyContent:"center", pointerEvents:"none" }}>
          <div style={{ width:"72%", aspectRatio:"63/88", border:`2px solid ${status==="found"?"#10b981":"rgba(245,158,11,.7)"}`, borderRadius:8, boxShadow:status==="found"?"0 0 20px rgba(16,185,129,.4)":"0 0 20px rgba(245,158,11,.2)", transition:"border-color .3s, box-shadow .3s" }} />
        </div>
        {/* Status bar */}
        <div style={{ position:"absolute", bottom:0, left:0, right:0, background:"linear-gradient(transparent,rgba(0,0,0,.85))", padding:"24px 14px 12px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        ...
        {lastCard && getImg(lastCard) && (
          <div style={{ position:"absolute", top:8, right:8, background:C.card, border:"2px solid #10b981", borderRadius:8, padding:3, animation:"fadeIn .2s ease", boxShadow:"0 4px 16px rgba(0,0,0,.6)" }}>
            <img src={getImg(lastCard)} alt="" style={{ width:48, borderRadius:5 }} />
          </div>
        )}
      </div>
      {status==="error" && (
        <div style={{ marginTop:10, background:`${C.red}15`, border:`1px solid ${C.red}33`, borderRadius:8, padding:"10px 12px", fontSize:12, color:C.red }}>❌ {errMsg}</div>
      )}
      ...
      <div style={{ marginTop:8, fontSize:11, color:C.muted, textAlign:"center", lineHeight:1.5 }}>
        Hold each card steady in frame. It'll vibrate and auto-add, then show the next card.
      </div>
    </div>
  );
};
...
const ScanTab = ({ onAdd, onSetCommander, format, activeDeck, deckCount, onSwitchDeck }) => {
  ...
  const handleFile = async (file) => {
    ...
    try {
      ...
      if (!parsed.name) throw new Error("Not recognised — try better lighting or angle");
      const sf = await fetch(`https://api.scryfall.com/cards/named?fuzzy=${encodeURIComponent(parsed.name)}`);
      const card = await sf.json();
  ...
  return (
    <div style={{ padding:"16px 16px 0" }}>
      ...
      <DeckSelectBar deck={activeDeck} deckCount={deckCount} onSwitch={onSwitchDeck} />

      {/* Flash */}
      {flash && (
        <div style={{ background:flash.startsWith("👑")?`${C.purple}20`:`${C.green}20`, border:`1px solid ${flash.startsWith("👑")?`${C.purple}44`:`${C.green}44`}`, borderRadius:10, padding:"10px 14px", fontSize:13, fontWeight:600, color:flash.startsWith("👑")?"#d8b4fe":C.green, marginBottom:12, animation:"fadeIn .2s ease" }}>
          {flash}
        </div>
      )}
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:6, marginBottom:12 }}>
        {[{id:"live",emoji:"🔴",label:"Live Scan"},{id:"photo",emoji:"📷",label:"Photo"},{id:"search",emoji:"🔍",label:"Search"}].map(m => (
          <button key={m.id} onClick={() => { setMode(m.id); if(m.id!=="live") setLive(false); }} style={{
            ...touch, padding:"10px 4px", borderRadius:10, border:`1px solid ${mode===m.id?C.gold+"66":C.border}`,
      ...
      {mode==="live" && (
        <Panel>
          ...
          {!liveActive ? (
            <>
              <div style={{ fontSize:13, color:C.sub, marginBottom:12, lineHeight:1.65 }}>
                Point your camera at any card — the app auto-detects it and adds it straight to <strong style={{ color:C.gold }}>{activeDeck.name}</strong>. Vibrates on each add. Then show the next card.
              </div>
              <Btn onClick={() => setLive(true)} style={{ width:"100%", display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
                🔴 Start Live Scanning
              </Btn>
            </>
          ) : (
            <LiveScan
              onAdd={c => { onAdd(c); showFlash(`✓ ${c.name} → ${activeDeck.name}`); }}
      ...
      {mode==="photo" && (
        <Panel>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:preview?12:0 }}>
            {[{label:"Take Photo",emoji:"📷",ref:cameraRef,cap:true},{label:"From Gallery",emoji:"🖼️",ref:galleryRef,cap:false}].map(({label,emoji,ref}) => (
                ...
                {scanState==="found" && result && (
                  <>
                    ...
                    <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                      <Btn onClick={() => { onAdd(result); showFlash(`✓ ${result.name} → ${activeDeck.name}`); reset(); }} style={{ fontSize:13, padding:"9px 14px" }}>
                        ✓ Add to {activeDeck.name}
                      </Btn>
                      {format==="commander" && isLegendary(result) && (
                        <Btn variant="purple" onClick={() => { onSetCommander(result); onAdd(result); showFlash(`👑 Commander: ${result.name}`); reset(); }} style={{ fontSize:13, padding:"9px 14px" }}>
                          👑 Set Commander
                        </Btn>
                      )}
                  ...
                  <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                    {format==="commander" && isLegendary(card) && (
                      <button onClick={() => { onSetCommander(card); onAdd(card); showFlash(`👑 Commander: ${card.name}`); setQuery(""); setSearchRes([]); }}
                        style={{ ...touch, fontSize:10, fontWeight:700, color:C.purple, background:`${C.purple}15`, border:`1px solid ${C.purple}44`, borderRadius:6, padding:"5px 8px", minHeight:32 }}>
                        👑
                      </button>
                    )}
                    <button onClick={() => { onAdd(card); showFlash(`✓ ${card.name} → ${activeDeck.name}`); setQuery(""); setSearchRes([]); }}
                      style={{ ...touch, fontSize:10, fontWeight:700, color:C.gold, background:`${C.gold}15`, border:`1px solid ${C.gold}33`, borderRadius:6, padding:"5px 10px", minHeight:32 }}>
                      + Add
                    </button>
                  </div>
                </div>
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 0", borderBottom:`1px solid ${C.border}22`, background:isCmd?"rgba(168,85,247,.05)":"transparent" }}>
      ...
      <div style={{ flex:1, minWidth:0 }}>
        ...
        {violation && <div style={{ fontSize:10, color:C.red, marginTop:2 }}>⚠️ Singleton rule</div>}
        {canCmd && (
          <button onClick={() => onSetCommander(card)} style={{ ...touch, marginTop:4, fontSize:10, fontWeight:700, color:C.purple, background:`${C.purple}15`, border:`1px solid ${C.purple}44`, borderRadius:5, padding:"3px 8px", minHeight:24 }}>
            👑 Make Commander
          </button>
        )}
      </div>
  ...
  return (
    <div style={{ position:"relative" }}>
      <div onClick={() => onPreview(card)} style={{ ...touch, borderRadius:8, overflow:"hidden", background:C.card2, border:`1px solid ${C.border}`, aspectRatio:"63/88" }}>
        {img ? <img src={img} alt={card.name} style={{ width:"100%", height:"100%", objectFit:"cover" }} />
             : <div style={{ width:"100%", height:"100%", display:"flex", alignItems:"center", justifyContent:"center", color:C.muted, fontSize:9, padding:4, textAlign:"center" }}>{card.name}</div>}
      </div>
      {count > 1 && (
        <div style={{ position:"absolute", top:4, right:4, background:C.gold, color:"#0a0f1e", borderRadius:10, minWidth:18, height:18, display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:800, padding:"0 4px" }}>{count}</div>
      )}
...
const DeckTab = ({ deck, onUpdate, onPreview }) => {
  ...
  const grouped = useMemo(() => {
    ...
    cards.forEach(entry => {
      ...
      (map[cat] ?? (map["other"] = map["other"]||[])).push(entry);
    });
    ...
    <div style={{ padding:"16px 16px 0" }}>
      <Panel>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8, gap:10 }}>
          <div>
            <div style={{ fontSize:18, fontWeight:800, color:C.gold }}>{deck.name}</div>
            <div style={{ fontSize:12, color:C.sub, marginTop:1 }}>{total}/{target} cards</div>
          </div>
        ...
        <div style={{ height:7, background:C.card2, borderRadius:4, overflow:"hidden" }}>
          <div style={{ height:"100%", width:`${progress}%`, borderRadius:4, transition:"width .4s", background:total>=target?`linear-gradient(90deg,${C.green},#34d399)`:format==="commander"?"linear-gradient(90deg,#a855f7,#7c3aed)":`linear-gradient(90deg,${C.gold},#d97706)` }} />
        </div>
        {violations.size>0 && (
          <div style={{ marginTop:8, background:`${C.red}15`, border:`1px solid ${C.red}33`, borderRadius:8, padding:"7px 10px", fontSize:12, color:C.red }}>
            ⚠️ {violations.size} singleton violation{violations.size>1?"s":""}
          </div>
        )}
        {deck.notes && (
          <div style={{ marginTop:10, fontSize:12, color:C.muted, lineHeight:1.5, fontStyle:"italic", borderTop:`1px solid ${C.border}`, paddingTop:10 }}>
            📝 {deck.notes}
          </div>
        )}
      </Panel>
      ...
      {format==="commander" && (
        <CommanderHeroes commanders={commanders} onRemove={removeCommander} onPreview={onPreview} canAddSecond={commanders.length===1} />
      )}
      {format==="commander" && !commanders.length && cards.length>0 && (
        <div style={{ background:`${C.purple}10`, border:`1px solid ${C.purple}33`, borderRadius:10, padding:"10px 14px", marginBottom:12, fontSize:13, color:"#d8b4fe" }}>
      ...
      {cards.length===0 ? (
        <div style={{ textAlign:"center", padding:"48px 0", color:C.muted, fontSize:14 }}>
          ...
          No cards yet — go to the Scan tab to add cards!
        </div>
      ) : viewMode==="grid" ? (
        <Panel>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8 }}>
            {[...cards].sort((a,b) => {
              ...
              if(ta==="Land"&&tb!=="Land") return 1; if(ta!=="Land"&&tb==="Land") return -1;
              return (a.card.cmc??0)-(b.card.cmc??0);
            }).map(entry => <CardThumb key={entry.card.id} entry={entry} onPreview={onPreview} onDelta={updateCount} />)}
          </div>
        </Panel>
      ) : (
        CARD_CATEGORIES.map(cat => {
          ...
          return (
            <div key={cat.id} style={{ marginBottom:8 }}>
              <button onClick={() => setCollapsed(p=>({...p,[cat.id]:!p[cat.id]}))} style={{ ...touch, width:"100%", display:"flex", alignItems:"center", justifyContent:"space-between", background:C.card, border:`1px solid ${C.border}`, borderRadius:open?"12px 12px 0 0":12, padding:"10px 14px", minHeight:44 }}>
              ...
              {open && (
                <div style={{ background:C.card, border:`1px solid ${C.border}`, borderTop:"none", borderRadius:"0 0 12px 12px", padding:"0 14px" }}>
                  {entries.map(entry => (
                    <CardRow key={entry.card.id} entry={entry} onDelta={updateCount} onPreview={onPreview} format={format} commanders={commanders} onSetCommander={setCommander} violation={violations.has(entry.card.id)} />
                  ))}
                </div>
              )}
            </div>
...
const StatsTab = ({ analytics, deck }) => {
  ...
  return (
    <div style={{ padding:"16px 16px 0" }}>
      <Panel>
        <SecTitle>Overview — {deck.name}</SecTitle>
        ...
        {landWarn && <div style={{ marginTop:10, background:"#f59e0b22", border:"1px solid #f59e0b44", borderRadius:8, padding:"8px 12px", fontSize:12, color:"#f59e0b" }}>⚠️ Lands at {landPct}% — recommended {minL}–{maxL}%</div>}
      </Panel>
      {colorData.length>0 && (
        <Panel>
        ...
        {typeData.map(([type,count]) => (
          <div key={type} style={{ marginBottom:9 }}>
            ...
            <div style={{ height:6, background:C.card2, borderRadius:3, overflow:"hidden" }}>
              <div style={{ height:"100%", width:`${Math.min(100,(count/total)*100)}%`, background:TYPE_COLORS[type]??C.muted, borderRadius:3, transition:"width .4s" }} />
            </div>
          </div>
        ))}
      </Panel>
...
const InsightsTab = ({ deck, analytics }) => {
  const [status,setStatus]=useState("idle"); const [insights,setInsights]=useState(""); const [bracket,setBracket]=useState(null); const [error,setError]=useState("");
  ...
  const generate = async () => {
    ...
    try {
      ...
      const prompt = `You are an expert MTG deckbuilder.\n\nFormat: ${isCmd?"Commander/EDH (100-card singleton)":"60-card"}\nDeck: "${deck.name||"Unnamed"}"\n${cmdLine}\nStats: Total:${total??0}, Lands:${lands??0}, AvgCMC:${avgCmc??0}\n\nDecklist:\n${deckList}\n\n**🎯 Archetype & Strategy**\n2-3 sentences.${isCmd?" Reference the commander.":""}\n\n**🏆 Bracket Estimate**\n${isCmd?`Bracket 1–5. Format: "Bracket X — Label"\n1=Exhibition,2=Core,3=Upgraded,4=Optimised,5=cEDH`:`Power 1–5. "Bracket X — Label"\n1=Budget,2=Casual,3=Competitive,4=PTQS,5=Pro Tour`}\n\n**💪 Key Strengths**\n- 3-4 strengths naming cards\n\n**⚠️ Weaknesses & Risks**\n- 3-4 vulnerabilities\n\n**💡 Recommended Changes**\n- Specific ADD/CUT suggestions (at least 4)\n\n**🏔️ Mana Base Assessment**\nLand count, fixing, ramp.\n\n**⭐ Overall Rating**\nX/10 with paragraph summary.`;
  ...
  return (
    <div style={{ padding:"16px 16px 0" }}>
      <Panel>
        <SecTitle>AI Deck Analysis — {deck.name}</SecTitle>
        ...
        {!API_KEY && <div style={{ background:`${C.red}15`, border:`1px solid ${C.red}33`, borderRadius:8, padding:"10px 12px", fontSize:12, color:C.red, marginBottom:12 }}>⚠️ Add VITE_ANTHROPIC_API_KEY in Vercel settings → redeploy</div>}
      ...
      {status==="done" && insights && <Panel><BracketBadge bracket={bracket}/>{renderMd(insights)}</Panel>}
    </div>
  );
};
...
export default function DeckScanner() {
  // ── Single combined state — fixes the ID-mismatch bug ──
  const [{ decks, activeId }, setGlobal] = useState(initState);
  const [tab,     setTab]     = useState("scan");
  const [preview, setPreview] = useState(null);

  const setDecks    = useCallback(fn => setGlobal(s => ({ ...s, decks: typeof fn==="function" ? fn(s.decks) : fn })), []);
  const setActiveId = useCallback(id  => setGlobal(s => ({ ...s, activeId: id })), []);
  ...
  const setCommander = useCallback((card) => {
    setDecks(prev => prev.map(d => {
      if (d.id !== activeId) return d;
      return { ...d, commanders: d.commanders.length>=2 ? [...d.commanders.slice(0,1),card] : [...d.commanders.filter(c=>c.id!==card.id),card] };
    }));
  }, [activeId, setDecks]);

  const setFormat = useCallback((fmt) => {
    setDecks(prev => prev.map(d => d.id!==activeId ? d : { ...d, format:fmt, commanders:fmt==="standard"?[]:d.commanders }));
  }, [activeId, setDecks]);

  const createDeck = useCallback(() => {
    const d = newDeck(`Deck ${decks.length+1}`);
    setGlobal(s => ({ decks:[...s.decks,d], activeId:d.id }));
    setTab("scan");
  }, [decks.length, setGlobal]);

  const updateDeckMeta = useCallback((id, patch) => {
    setDecks(prev => prev.map(d => d.id===id ? {...d,...patch} : d));
  }, [setDecks]);

  const deleteDeck = useCallback((id) => {
    setGlobal(s => {
      const next = s.decks.filter(d => d.id!==id);
      return { decks:next, activeId: s.activeId===id ? next[0]?.id : s.activeId };
    });
  }, [setGlobal]);

  const switchDeck = useCallback((id) => {
    setActiveId(id);
    setTab("scan");
  }, [setActiveId]);
  ...
  const analytics = useMemo(() => {
    ...
    const cc={W:0,U:0,B:0,R:0,G:0}; const tc={};
    let lands=0,totalCmc=0,spells=0;
    cards.forEach(({card,count})=>{
      const type=getCardType(card.type_line); tc[type]=(tc[type]||0)+count;
      if(type==="Land"){lands+=count;return;}
      const cmc=card.cmc||0; const key=cmc>=7?"7+":String(Math.floor(cmc));
  ...
  return (
    <div style={{ maxWidth:480, margin:"0 auto", minHeight:"100dvh", background:C.bg, display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ position:"sticky", top:0, zIndex:200, background:`${C.card}f0`, backdropFilter:"blur(12px)", WebkitBackdropFilter:"blur(12px)", borderBottom:`1px solid ${C.border}`, padding:"10px 14px", display:"flex", alignItems:"center", gap:10 }}>
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ fontSize:17, fontWeight:800, color:C.gold, letterSpacing:-.3 }}>⚔️ MTG Companion</div>
          <div style={{ fontSize:10, color:C.muted, marginTop:1 }}>{decks.length} deck{decks.length!==1?"s":""} · tap 🗂 Decks to manage</div>
        </div>
      ...
      <div style={{ flex:1, overflowY:"auto", paddingBottom:72 }}>
        {tab==="scan"  && <ScanTab onAdd={addCard} onSetCommander={setCommander} format={activeDeck.format} activeDeck={activeDeck} deckCount={decks.length} onSwitchDeck={() => setTab("decks")} />}
        {tab==="deck"  && <DeckTab deck={activeDeck} onUpdate={updateDeck} onPreview={setPreview} />}
        {tab==="decks" && <DecksTab decks={decks} activeId={activeId} onSwitch={switchDeck} onCreate={createDeck} onUpdate={updateDeckMeta} onDelete={deleteDeck} />}
        {tab==="stats" && <StatsTab analytics={analytics} deck={activeDeck} />}
        {tab==="ai"    && <InsightsTab deck={activeDeck} analytics={analytics} />}
      </div>

      {/* Tab bar */}
      <div style={{ position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:480, background:`${C.card}fa`, backdropFilter:"blur(16px)", WebkitBackdropFilter:"blur(16px)", borderTop:`1px solid ${C.border}`, display:"grid", gridTemplateColumns:"repeat(5,1fr)", zIndex:100, paddingBottom:"max(env(safe-area-inset-bottom,0px),4px)" }}>
        {TABS.map(t => {
          const active = tab===t.id;
          return (
            <button key={t.id} onClick={() => setTab(t.id)} style={{ ...touch, padding:"8px 2px 6px", display:"flex", flexDirection:"column", alignItems:"center", gap:2, color:active?C.gold:C.muted, background:"none", border:"none", position:"relative", minHeight:52 }}>
              {active && <div style={{ position:"absolute", top:0, left:"15%", right:"15%", height:2, background:C.gold, borderRadius:"0 0 3px 3px" }} />}
              <span style={{ fontSize:20 }}>{t.emoji}</span>
              <span style={{ fontSize:9, fontWeight:active?700:400, letterSpacing:.2 }}>{t.label}</span>
            </button>
          );
        })}
      </div>
